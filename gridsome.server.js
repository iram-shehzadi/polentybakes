...
module.exports = function (api) {
	...
	api.createManagedPages(async ({ createPage }) => {
		...
		const getTranslations = async () => {
			...
			response.data.forEach((translation) => {
				...
				Utils.Objects.assign(
					translations,
					translationData.lang + '.categories',
					JSON.parse(translationData.translations)
				);
			});
			return translations;
		};
		...
		const getProducts = async () => {
			const response = await client.query(
				q.Map(q.Paginate(q.Match(q.Index('allProducts'))), (item) =>
					q.Get(item)
				)
			);
		...
		const getCategoryProducts = (products) => {
			...
			products.forEach((product) => {
				const productData = product.data;
				let category = Utils.Objects.find(
					categories,
					productData.categoryKeyPath
				);
		...
		fs.writeFileSync(
			'./static/category/products.json',
			JSON.stringify(categoryProducts)
		);
		fs.writeFileSync(
			'./static/category/translations.json',
			JSON.stringify(translations)
		);

		products.forEach((product) => {
			fs.writeFileSync(
				'./static/category/product-' + product.data.id + '.json',
